...
def _check_email_success(response):
    for email, success in response.items():
        if not success:
...
class User(object):
    ...
    def exists(self, *emails):
        ...
        return dict([(hashes[hash], found==1)
                   for hash, found in self._call('exists', hashes=list(hashes.keys())).items()])
    ...
    def emails(self):
        ...
        return dict([(email, Image(id=userimage['userimage'], url=userimage['userimage_url'],
                   ...
                   for email, userimage in self._call('addresses').items()])
    ...
    def images(self):
        ...
        return [Image(id=id, url=url, rating=int(rating))
                   for id, (rating, url) in self._call('userimages').items()]
